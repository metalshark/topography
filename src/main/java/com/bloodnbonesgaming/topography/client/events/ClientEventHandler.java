...
public class ClientEventHandler {
	...
	public void OnOpenGui(final GuiOpenEvent event) {
		...
		final CreateWorldScreen createWorldScreen = (CreateWorldScreen) gui;
		//Initialize presets so they exist for the gui
		ConfigurationManager.getGlobalConfig().initPresets();
		event.setGui(GuiCreateWorld.func_243425_a(createWorldScreen.parentScreen));
	}
	...
	public void OnWorldTick(final WorldTickEvent event) {
		...
		if (world.isRemote)
			return;
		final ServerWorld serverWorld = (ServerWorld) event.world;
		...
		final IntegratedServer integratedServer = Minecraft.getInstance().getIntegratedServer();
		if (integratedServer == null)
			return;

		for (final Entry<ResourceLocation, DimensionDef> entry : preset.defs.entrySet()) {
			final DimensionDef def = entry.getValue();
			if (def.spawnStructure == null)
				continue;

			final RegistryKey<World> worldKey = RegistryKey.getOrCreateKey(Registry.WORLD_KEY, entry.getKey());

			final ServerWorld dimWorld = integratedServer.getWorld(worldKey);
			if (dimWorld == null)
				continue;

			int preloadArea = def.spawnStructure.getSize().getX();
			preloadArea = Math.max(def.spawnStructure.getSize().getZ(), preloadArea);
			...
			final BlockPos pos = new BlockPos(0, def.spawnStructureHeight, 0);
			...
			if (!entry.getKey().equals(World.OVERWORLD.getLocation()))
				continue;
	...
	public void OnServerAboutToStart(FMLServerAboutToStartEvent event) {
		...
		final String line = FileHelper.readLineFromFile(server.anvilConverterForAnvilFile.getWorldDir() + "/topography.txt");//TODO Check file existence first

		if (line == null)
			return;
		Topography.getLog().info("Read preset: " + line);

		final ServerWorldInfo serverConfig = ((ServerWorldInfo) server.serverConfig);
		final long seed = serverConfig.generatorSettings.getSeed();
		...
		final SimpleRegistry<Dimension> registry = serverConfig.generatorSettings.func_236224_e_();
		...
		for (final Entry<ResourceLocation, DimensionDef> entry : preset.defs.entrySet()) {
			...
			final RegistryKey<Dimension> dimensionRegistryKey = RegistryKey.getOrCreateKey(Registry.DIMENSION_KEY, worldType);
			final Dimension oldDim = registry.getValueForKey(dimensionRegistryKey);
			...
			if (oldDim != null) {
				...
				server.field_240767_f_.getRegistry(Registry.DIMENSION_TYPE_KEY).register(RegistryKey.getOrCreateKey(Registry.DIMENSION_TYPE_KEY, entry.getKey()), dimType, Lifecycle.stable());
				typeSupplier = () -> {
			...
			registry.register(dimensionRegistryKey, new Dimension(typeSupplier, chunkGen), Lifecycle.stable());
		}
		serverConfig.generatorSettings = new DimensionGeneratorSettings(seed, true, false, registry);
	}
