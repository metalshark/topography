...
public class LockPresetCommand implements ICommand 
{
    ...
    {
        return "Use /topography lock <presetName>";
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        if (args.length > 0)
        {
            ...
            if (preset != null)
            {
                ConfigurationManager.getInstance().lockPreset(preset.getName());
                sender.sendMessage(new TextComponentString("Topography preset `" + preset.getName() + "' locked!"));
            }
        }
    }
