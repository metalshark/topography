...
public class StructureTemplate extends Template {
      ...
      if (size.getX() >= 1 && size.getY() >= 1 && size.getZ() >= 1) {
         ...
         for(BlockPos blockpos3 : BlockPos.getAllInBoxMutable(blockpos1, blockpos2)) {
            ...
            if (toIgnore == null || toIgnore != blockstate.getBlock()) {
               ...
               StructureTemplate.BlockInfo template$blockinfo;
               if (tileentity != null) {
                  CompoundNBT compoundnbt = tileentity.write(new CompoundNBT());
                  compoundnbt.remove("x");
                  compoundnbt.remove("y");
                  compoundnbt.remove("z");
                  template$blockinfo = new StructureTemplate.BlockInfo(blockpos4, blockstate, compoundnbt.copy());
               } else {
                  template$blockinfo = new StructureTemplate.BlockInfo(blockpos4, blockstate, (CompoundNBT)null);
               }
   ...
   private static void func_237149_a_(StructureTemplate.BlockInfo p_237149_0_, List<StructureTemplate.BlockInfo> p_237149_1_, List<StructureTemplate.BlockInfo> p_237149_2_, List<StructureTemplate.BlockInfo> p_237149_3_) {
   ...
   private static List<Template.BlockInfo> func_237151_a_(List<StructureTemplate.BlockInfo> p_237151_0_, List<StructureTemplate.BlockInfo> p_237151_1_, List<StructureTemplate.BlockInfo> p_237151_2_) {
   ...
   private void takeEntitiesFromWorld(World worldIn, BlockPos startPos, BlockPos endPos) {
      ...
      for(Entity entity : list) {
         ...
         this.entities.add(new StructureTemplate.EntityInfo(vector3d, blockpos, compoundnbt.copy()));
      }
      ...
      if (this.blocks.isEmpty()) {
         ...
         for(Template.BlockInfo template$blockinfo : p_215386_2_.func_237132_a_(this.blocks, p_215386_1_).func_237158_a_(p_215386_3_)) {
            ...
            if (mutableboundingbox == null || mutableboundingbox.isVecInside(blockpos)) {
               list.add(new StructureTemplate.BlockInfo(blockpos, template$blockinfo.state.rotate(p_215386_2_.getRotation()), template$blockinfo.nbt));
            }
         }
   ...
   public boolean func_237146_a_(IServerWorld p_237146_1_, BlockPos p_237146_2_, BlockPos p_237146_3_, PlacementSettings p_237146_4_, Random p_237146_5_, int p_237146_6_) {
      if (this.blocks.isEmpty()) {
         ...
         if ((!list.isEmpty() || !p_237146_4_.getIgnoreEntities() && !this.entities.isEmpty()) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
        	...
        	List<Template.EntityInfo> extraEntities = new ArrayList<Template.EntityInfo>();

            for(Template.BlockInfo template$blockinfo : processBlockInfos(p_237146_1_, p_237146_2_, p_237146_3_, p_237146_4_, list, this, extraEntities)) {
               BlockPos blockpos = template$blockinfo.pos;

               if (mutableboundingbox == null || mutableboundingbox.isVecInside(blockpos)) {
//                	 Topography.getLog().info("Block " + blockpos);
            ...
            if (!p_237146_4_.getIgnoreEntities()) {
               this.addEntitiesToWorld(p_237146_1_, p_237146_2_, p_237146_4_, extraEntities);
            }
   ...
   public static List<Template.BlockInfo> processBlockInfos(IWorld p_237145_0_, BlockPos p_237145_1_, BlockPos p_237145_2_, PlacementSettings p_237145_3_, List<Template.BlockInfo> p_237145_4_, @Nullable StructureTemplate template, List<Template.EntityInfo> extraEntities) {
      List<Template.BlockInfo> list = Lists.newArrayList();
      ...
      for(Template.BlockInfo template$blockinfo : p_237145_4_) {
         ...
         Template.BlockInfo template$blockinfo1 = new StructureTemplate.BlockInfo(blockpos, template$blockinfo.state, template$blockinfo.nbt != null ? template$blockinfo.nbt.copy() : null);
         Iterator<StructureProcessor> iterator = p_237145_3_.getProcessors().iterator();
         
         while (iterator.hasNext() && template$blockinfo1 != null) {
        	 StructureProcessor processor = iterator.next();
        	 int entityCount = extraEntities.size();
        	 if (processor instanceof TStructureProcessor) {
        		 Template.BlockInfo ret = ((TStructureProcessor)processor).process(p_237145_0_, p_237145_1_, p_237145_2_, template$blockinfo, template$blockinfo1, p_237145_3_, template, extraEntities);
        		 if (entityCount < extraEntities.size()) {
//            		 Topography.getLog().info(extraEntities.get(extraEntities.size() - 1) + " " + template$blockinfo1);
            	 }
        		 template$blockinfo1 = ret;
        	 } else {
            	 template$blockinfo1 = processor.process(p_237145_0_, p_237145_1_, p_237145_2_, template$blockinfo, template$blockinfo1, p_237145_3_, template);
        	 }
         }

//         for(Iterator<StructureProcessor> iterator = p_237145_3_.getProcessors().iterator(); template$blockinfo1 != null && iterator.hasNext(); template$blockinfo1 = iterator.next().process(p_237145_0_, p_237145_1_, p_237145_2_, template$blockinfo, template$blockinfo1, p_237145_3_, template, extraEntities)) {
//         }
   ...
   public static List<Template.EntityInfo> processEntityInfos(@Nullable StructureTemplate template, IWorld worldIn, BlockPos offsetPos, PlacementSettings placementSettingsIn, List<Template.EntityInfo> blockInfos, List<Template.EntityInfo> extraEntities) {
      ...
      for(Template.EntityInfo entityInfo : blockInfos) {
         ...
         Template.EntityInfo info = new StructureTemplate.EntityInfo(pos, blockpos, entityInfo.nbt);
...
//      Topography.getLog().info("Extra entities: " + extraEntities.size());
      for(Template.EntityInfo entityInfo : extraEntities) {
          Vector3d pos = transformedVec3d(placementSettingsIn, entityInfo.pos.add(0.5, 0, 0.5)).add(Vector3d.copy(offsetPos));
          BlockPos blockpos = transformedBlockPos(placementSettingsIn, entityInfo.blockPos).add(offsetPos);
          Template.EntityInfo info = new StructureTemplate.EntityInfo(pos, blockpos, entityInfo.nbt);
          for (StructureProcessor proc : placementSettingsIn.getProcessors()) {
             info = proc.processEntity(worldIn, offsetPos, entityInfo, info, placementSettingsIn, template);
          ...
          if (info != null)
             list.add(info);
//          Topography.getLog().info("Entity: " + pos + " " + blockpos);
       }
      return list;
   }

   private void addEntitiesToWorld(IServerWorld p_237143_1_, BlockPos p_237143_2_, PlacementSettings placementIn, List<Template.EntityInfo> extraEntities) {
      for(Template.EntityInfo template$entityinfo : processEntityInfos(this, p_237143_1_, p_237143_2_, placementIn, this.entities, extraEntities)) {
...
//        	 Topography.getLog().info("Trying to spawn entity at " + blockpos);
...
//          	 Topography.getLog().info("Entity loaded");
            });
         }
      }
...
//      }
   ...
   public CompoundNBT writeToNBT(CompoundNBT nbt) {
      if (this.blocks.isEmpty()) {
         ...
         List<StructureTemplate.BasicPalette> list = Lists.newArrayList();
         StructureTemplate.BasicPalette template$basicpalette = new StructureTemplate.BasicPalette();
         list.add(template$basicpalette);

         for(int i = 1; i < this.blocks.size(); ++i) {
            list.add(new StructureTemplate.BasicPalette());
         }
         ...
         for(int j = 0; j < list1.size(); ++j) {
            ...
            for(int l = 1; l < this.blocks.size(); ++l) {
               StructureTemplate.BasicPalette template$basicpalette1 = list.get(l);
         ...
         if (list.size() == 1) {
            ...
            for(StructureTemplate.BasicPalette template$basicpalette2 : list) {
   ...
   public void read(CompoundNBT compound) {
      ...
      for(int j = 0; j < listnbt5.size(); ++j) {
         ...
         if (compoundnbt.contains("nbt")) {
            ...
            this.entities.add(new StructureTemplate.EntityInfo(vector3d, blockpos, compoundnbt1));
         }
      }
   ...
   private void readPalletesAndBlocks(ListNBT palletesNBT, ListNBT blocksNBT) {
      ...
      for(int j = 0; j < blocksNBT.size(); ++j) {
         ...
         StructureTemplate.BlockInfo template$blockinfo = new StructureTemplate.BlockInfo(blockpos, blockstate, compoundnbt1);
   ...
   public static class BlockInfo extends Template.BlockInfo {
   ...
   public static class EntityInfo extends Template.EntityInfo {

      public EntityInfo(Vector3d vecIn, BlockPos posIn, CompoundNBT nbt) {
    	  super(vecIn, posIn, nbt);
      }
   }

//   public static final class Palette {
//      private final List<StructureTemplate.BlockInfo> field_237155_a_;
//      private final Map<Block, List<StructureTemplate.BlockInfo>> field_237156_b_ = Maps.newHashMap();
//
//      private Palette(List<StructureTemplate.BlockInfo> p_i232120_1_) {
//         this.field_237155_a_ = p_i232120_1_;
//      }
//
//      public List<StructureTemplate.BlockInfo> func_237157_a_() {
//         return this.field_237155_a_;
//      }
//
//      public List<StructureTemplate.BlockInfo> func_237158_a_(Block p_237158_1_) {
