...
public class VanillaNoiseGeneratorOctaves extends NoiseGeneratorOctaves {

	public VanillaNoiseGeneratorOctaves(Random seed, int octavesIn) {
	...
	public double[] generateNoiseOctaves(double[] noiseArray, int xOffset, int zOffset, int xSize, int zSize,
		...
		//Pass 2D noise off to the super instead of multithreading pointlessly
        return super.generateNoiseOctaves(noiseArray, xOffset, 10, zOffset, xSize, 1, zSize, xScale, 1.0D, zScale);
	}
	...
	public double[] generateNoise(double[] noiseArray, int xOffset, int yOffset, int zOffset, int xSize, int zSize, double xScale, double yScale, double zScale, int yIndex) {
        ...
        for (int j = 0; j < this.octaves; ++j)
        {
            ...
            d0 = d0 + (double)k;
            d2 = d2 + (double)l;
            this.generatorCollection[j].populateNoiseArray(noiseArray, d0, d1 + (double)yIndex * (yScale * d3), d2, xSize, 1, zSize, xScale * d3, yScale * d3, zScale * d3, d3);
            d3 /= 2.0D;
        }
